...
def load_openapi_spec(file_path: str, logger: logging.Logger) -> dict:
        ...
        if not path.startswith(("/", "./")):
            ...
            if not path.startswith("openapi/"):
                possible_paths.append(
                    os.path.join(parent_dir, "openapi", os.path.basename(path))
                )
    ...
    if actual_file_path.endswith(".yaml") or actual_file_path.endswith(".yml"):
        json_file_path = actual_file_path.replace(".yaml", ".json").replace(
            ".yml", ".json"
        )
    ...
    with open(actual_file_path, "r") as f:
        ...
        if "paths" in spec:
            ...
            logger.info(
                f"Using full OpenAPI spec with {len(paths)} paths and approximately {total_routes} routes (cleaned up)"
            )
    ...
    for path, methods in openapi_spec["paths"].items():
        for method, operation in methods.items():
            if method.lower() in [
                ...
                if operation_id and (
                    operation_id.startswith("v1") or operation_id.startswith("V1")
                ):
